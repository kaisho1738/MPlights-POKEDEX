...
/*
	...
	----- Changes -----
	- removed loop to make all upper sa line 120; instead i implemented a case insensitivity sa search pokemon [as required sa specs]
	- added & sa dtemp1 [scanf]
	- added option to delete by name sa function deletepoke mo; implemented case insensitivity din para if pokemon is Tite; it can still find even if TITE
	- search and del pokemo is now fully working
	- fixed chart not displaying type2 properly; parehong temp1 nalagay, so i made the other temp1 -> temp2
	---- RANDOM CODE NOTES ----
	- lagay nalang here mga need pa gawin or mga error na di naresolve
	- [thaeron] function 8 is still not done
	- [fix formatting function 5] [make the searchPokemonfunction not case sensitive]
...
void searchPokemon(pokeData myPokemons[], int size, char *pokemonName, int pokemonDexNum) {
	int i, flag = 0, choice;
	char toFind[50] = {0}, pokemonEntered[50] = {0};
	int toFindLength = strlen(myPokemons[i].pokeName);
	int pokemonEnteredLength =  strlen(pokemonName);
	printBorder();
    printf("Searching Pokemon...\n");
	printBorder();
	printf("Would you like to view the Pokemon via [0]Chart or [1]Table?: ");
	scanf("%d", &choice);
	
	for (i = 0; i < size; i++) {

		for(int k = 0; k < toFindLength; k++){ //for case insensitivity
			toFind[k] = tolower(myPokemons[i].pokeName[k]);
		}
		toFind[toFindLength] = '\0';

		for(int k = 0; k < pokemonEnteredLength; k++){ //for case insensitivity
			pokemonEntered[k] = tolower(pokemonName[k]);
		}
		pokemonEntered[pokemonEnteredLength] = '\0';

		if (strcmp(toFind, pokemonEntered) == 0 || myPokemons[i].dexNum == pokemonDexNum) {
			printBorder();
			printf("\nPokemon is Found!\n");
			printBorder();
			if(choice == 1){
			...
			displayPokemonChart(&myPokemons[i], size, pokemonTypes);
			}
			flag = 1;
		}
	} 
