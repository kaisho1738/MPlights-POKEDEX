...
/*
	...
	Last modified: 3/29/2025 7:09 | kim
	Version: 2.5
	...
	----- Changes -----
	- fixed search pokemon full working now [including search by type and evolves to/from]
	- added 2 fuctions = simulate battle (now working can check nalang din para sure) + helper function para dyan
	...
	- ERROR DETECTIOONS NALANG POGI
	

	--- REMAINING TO DO ----
	- error handling
	- file access sa delete
	- save battle function, edit pokemon function, sort table function 
	- baka may na miss out ako
...
//array without embedded colors
const char *pokemonTypes2[] = {"None", "Bug", "Dark", "Dragon", "Electric", "Fairy", "Fighting", "Fire", "Flying", "Ghost", "Grass",
...
void searchPokemon(pokeData myPokemons[], int size) {
	...
	int i, j, flag = 0;
	int viewOption;
	int placeHolderIndex, placeHolderIndex2;

	int searchPokeChoice = 0;
	int searchPokeDex = 0;
	int searchPokeType = 0;
	char searchPokeName[50];
	int indexNumbers[1000];
	int counter = 0;
	
	char ctemp1[50] = {0}; // temporary string storage
	char ctemp2[50] = {0}; // temporary string storage
	...
	switch (searchPokeChoice) {
		...
		case 3: displayPokemonType();
				printf("Enter Pokemon Type to Search [Use Number Assigned]: ");
				scanf("%d", &searchPokeType);
				printf("\033[2J\033[H");
				break;
	...
	for (i = 0; i < size; i++) {
        ...
        if ((searchPokeChoice == 1 && myPokemons[i].dexNum == searchPokeDex) || 
            ...
            flag = 1;
            printf("\033[2J\033[H");
			placeHolderIndex = myPokemons[i].dexNum;
			placeHolderIndex2 = i;
        }

}

	for(int i = 0; i < size; i++){ //searches evolves from
		if ((searchPokeChoice == 1 && myPokemons[i].dexNum == myPokemons[placeHolderIndex2].evolvesFrom) ||
		   (searchPokeChoice == 2 && myPokemons[i].dexNum == myPokemons[placeHolderIndex2].evolvesFrom)){
		flag = 1;
		printBorder();
		printf("Evolves from: ");
	...
	if(flag){
		...
		for(int i = 0; i < size; i++){
			if(myPokemons[i].dexNum == placeHolderIndex){
				displayPokemonChart(&myPokemons[i], 1, pokemonTypes);
			}
		}
	} 
	
	for(int i = 0; i < size; i++){ //searches evolves to
		if((searchPokeChoice == 1 && myPokemons[i].dexNum == myPokemons[placeHolderIndex2].evolvesTo) ||
		  (searchPokeChoice == 2 && myPokemons[i].dexNum == myPokemons[placeHolderIndex2].evolvesTo)) {
		flag = 1;
		printBorder();
		printf("Evolves to: ");
		displayPokemonChart(&myPokemons[i], 1, pokemonTypes);
	...
	//The following codes are to handle case 3: Pokemon Types
	for(i = 0; i < size; i++){
		if((searchPokeChoice == 3 && searchPokeType == myPokemons[i].pokeType1) ||
			searchPokeChoice == 3 && searchPokeType == myPokemons[i].pokeType2){
				flag = 1;
				indexNumbers[counter] = i;
				counter++;
			}
	}
	
	if(flag && searchPokeChoice == 3){
		printf("View Pokemon(s) via Chart[0] or Table[1]? ");
		scanf("%d", &viewOption);

		for(j = 0; j < counter; j++){
			int index = indexNumbers[j];
			if(viewOption == 1) displayPokemonTable(&myPokemons[index], 1, pokemonTypes);
			else if (viewOption == 0) displayPokemonChart(&myPokemons[index], 1, pokemonTypes);
		}
	}
...
int isWeakTo(int firstType, int secondType) {
    switch (firstType) {
        case 1: return (secondType == 7 || secondType == 8 || secondType == 16);
        case 2: return (secondType == 6 || secondType == 5 || secondType == 1);
        case 3: return (secondType == 12 || secondType == 3 || secondType == 5);
        case 4: return (secondType == 11);
        case 5: return (secondType == 17 || secondType == 14);
        case 6: return (secondType == 8 || secondType == 15 || secondType == 5);
        case 7: return (secondType == 18 || secondType == 16 || secondType == 11);
        case 8: return (secondType == 4 || secondType == 16 || secondType == 11);
        case 9: return (secondType == 9 || secondType == 2);
        case 10: return (secondType == 7 || secondType == 12 || secondType == 14 || secondType == 8);
        case 11: return (secondType == 18 || secondType == 12 || secondType == 10);
        case 12: return (secondType == 7 || secondType == 6 || secondType == 16 || secondType == 17);
        case 13: return (secondType == 6);
        case 14: return (secondType == 15 || secondType == 11);
        case 15: return (secondType == 1 || secondType == 2 || secondType == 9);
        case 16: return (secondType == 18 || secondType == 10 || secondType == 6 || secondType == 11 || secondType == 17);
        case 17: return (secondType == 7 || secondType == 6 || secondType == 11);
        case 18: return (secondType == 4 || secondType == 10);
        default: return 0;
    }
}
...
void simulatePokeBattle (pokeData myPokemons[], int size) {
	...
	puts("|                                                     TYPE EFFECTS                                                     |");
	puts("+--------------+---------------------------------------------------+---------------------------------------------------+");
	puts("|     Type     |                Strong Against                     |                  Weak Against                     |");
	puts("+--------------+---------------------------------------------------+---------------------------------------------------+");
    ...
    int pokeBattle1 = 0, pokeBattle2 = 0;
	int indexNum1 = 0, indexNum2 = 0;
	int hp = 0, def = 0, atk = 0, spd = 0, sp = 0, avg = 0, total = 0; //for pokemon1
    int hp2 = 0, def2 = 0, atk2 = 0, spd2 = 0, sp2 = 0, avg2 = 0, total2 = 0; //for pokemon 2
	int avgPlaceHolder = 0, avgPlaceHolder2 = 0;

	int firstPokeType = 0, secondPokeType = 0;
	int weaknessPresent = 0, weaknessPresent2 = 0;

	firstPokeType = myPokemons[indexNum1].pokeType1;
	secondPokeType = myPokemons[indexNum2].pokeType2;
	...
	//find pokemon
	for(int i = 0; i < size; i++){
		if(pokeBattle1 == myPokemons[i].dexNum){
			indexNum1 = i;
		}
		if(pokeBattle2 == myPokemons[i].dexNum){
			indexNum2 = i;
		}
	}

	hp = myPokemons[indexNum1].HP;
	atk = myPokemons[indexNum1].Atk;
	def = myPokemons[indexNum1].Def;
	spd = myPokemons[indexNum1].Spd;
	sp = myPokemons[indexNum1].Sp;

	hp2 = myPokemons[indexNum2].HP;
	atk2 = myPokemons[indexNum2].Atk;
	def2 = myPokemons[indexNum2].Def;
	spd2 = myPokemons[indexNum2].Spd;
	sp2 = myPokemons[indexNum2].Sp;

	weaknessPresent = isWeakTo(myPokemons[indexNum1].pokeType1, myPokemons[indexNum2].pokeType1);
	weaknessPresent2 = isWeakTo(myPokemons[indexNum2].pokeType1, myPokemons[indexNum1].pokeType1);
	...
	if(weaknessPresent){
		...
		sp = (int)(sp * 0.8);
		printBorder();
		printf("%s has a type advantage\n", myPokemons[indexNum2].pokeName);
		printf("%s -> %s\n", pokemonTypes[myPokemons[indexNum2].pokeType1], pokemonTypes[myPokemons[indexNum1].pokeType1]);
	} else if(weaknessPresent2){
		hp2 = (int)(hp * 0.8);
		atk2 = (int)(hp * 0.8);
		def2 = (int)(hp * 0.8);
		spd2 = (int)(spd * 0.8);
		sp2 = (int)(sp * 0.8);
		printBorder();
		printf("%s has a type advantage\n", myPokemons[indexNum1].pokeName);
		printf("%s -> %s\n", pokemonTypes[myPokemons[indexNum1].pokeType1], pokemonTypes[myPokemons[indexNum2].pokeType1]);
	}

	avgPlaceHolder = (0.10 * hp) + (0.30 * atk) + (0.20 * def)
	+ (0.15 * spd) + (0.15 * sp);

	avgPlaceHolder2 = (0.10 * hp2) + (0.30 * atk2) + (0.20 * def2)
	+ (0.15 * spd2) + (0.15 * sp2);

	//compare
	if(myPokemons[indexNum1].HP > myPokemons[indexNum2].HP) hp = 1, hp2 = 0;
	else hp2 = 1, hp = 0;

	if(myPokemons[indexNum1].Def > myPokemons[indexNum2].Def) def = 1, def2 = 0;
	else def2 = 1, def = 0;

	if(myPokemons[indexNum1].Atk > myPokemons[indexNum2].Atk) atk = 1, atk2 = 0;
	else atk2 = 1, atk = 0;

	if(myPokemons[indexNum1].Spd > myPokemons[indexNum2].Spd) spd = 1, spd2 = 0;
	else spd2 = 1, spd = 0;

	if(myPokemons[indexNum1].Sp > myPokemons[indexNum2].Sp) sp = 1, sp2 = 0;
	else sp2 = 1, sp = 0;

	if(avgPlaceHolder > avgPlaceHolder2) avg = 2, avg2 = 0;
	else avg2 = 2, avg = 0;

	total = hp + atk + def + spd + sp + avg;	
	total2 = hp2 + atk2 + def2 + spd2 + sp2 + avg2;
	printBorder();

	puts("+-----------------------------------------------------------------------+");
	puts("|                            STATISTICS TALLY                           |");
	puts("+----------------------+------+------+------+------+------+------+------+");
	puts("|        Pokemon       |  HP  |  Atk |  Def |  Spd |  Sp  |  Avg |  Tot |");
	puts("+----------------------+------+------+------+------+------+------+------+");
	printf("| %-20s |  %-3d |  %-3d |  %-3d |  %-3d |  %-3d |  %-3d |  %-3d |\n",
		   myPokemons[indexNum1].pokeName, hp, atk, def, spd, sp, avg, total);
	printf("| %-20s |  %-3d |  %-3d |  %-3d |  %-3d |  %-3d |  %-3d |  %-3d |\n",
		   myPokemons[indexNum2].pokeName, hp2, atk2, def2, spd2, sp2, avg2, total2);
	puts("+----------------------+------+------+------+------+------+------+------+");
	printBorder();
	displayPokemonChart(&myPokemons[indexNum1], 1, pokemonTypes);
	displayPokemonChart(&myPokemons[indexNum2], 1, pokemonTypes);

	if(total > total2){
		printf("%s is stronger than %s\n", myPokemons[indexNum1].pokeName, myPokemons[indexNum2].pokeName);
	} else if (total < total2){
		printf("%s is stronger than %s\n", myPokemons[indexNum2].pokeName, myPokemons[indexNum1].pokeName);
	} else if (total == total2){
		printf("%s and %s have equal strength\n", myPokemons[indexNum1].pokeName, myPokemons[indexNum2].pokeName);
	}
	printBorder();
}
